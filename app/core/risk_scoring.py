...
Produces a documented, transparent risk score (0–100) for each alert.

Formula (documented):
  base_score      = model_confidence * 40   (max 40pts)
  severity_score  = severity_rank * 20      (max 40pts: CRITICAL=40, HIGH=30, MEDIUM=15, LOW=5)
  correlation     = min(corr_count * 10, 15) (max 15pts)
  persistence     = persistence_bonus        (0-5pts)

  risk_score = clamp(base_score + severity_score + correlation + persistence, 0, 100)
"""
...
# Threat type weight multipliers (some threats inherently riskier)
